...
import {
  ...
  ShieldCheck, ArrowUpRight, Clock, Target,
  RefreshCw
} from 'lucide-react';
import { cn } from '@/lib';
...
const AI_LOGS = [
  ...
  'Validando isolamento por usuário...',
  'Lendo Google Agenda...',
  'Categorizando despesas...',
  'Tudo pronto.',
];

const PRIVATE_AI_STATES = [
  'Preparando ambiente...',
  'Isolando sessão...',
...
export function HeroInteligente({ isCompact = false }: HeroInteligenteProps) {
  ...
  const [privateAIIndex, setPrivateAIIndex] = useState(0);
  ...
  useEffect(() => {
    ...
    const sequence = [
      ...
      { step: 5, delay: 1700 },  // Ambiente privado fica pronto
  ...
  // Inicializa o ambiente privado da IA
  useEffect(() => {
    if (wowStep >= 5) {
      let step = 0;
      const interval = setInterval(() => {
        if (step < PRIVATE_AI_STATES.length - 1) {
          step++;
          setPrivateAIIndex(step);
        } else {
          ...
          <div className={cn(
            ...
            "hover:-translate-y-1 hover:shadow-[0_15px_40px_rgba(16,185,129,0.15)]"
             ...
             <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-[13px] font-bold text-[#A7F3D0] truncate">IA Privada</span>
                  <span className="text-[11px] font-medium text-[#6EE7B7] flex items-center gap-1 truncate">
                    {wowStep >= 5 && <span className={cn(
                      ...
                      privateAIIndex === PRIVATE_AI_STATES.length - 1 ? "bg-[#34D399] shadow-[0_0_5px_#34D399]" : "bg-yellow-400 animate-pulse"
                    )} />}
                    {wowStep >= 5 ? PRIVATE_AI_STATES[privateAIIndex] : "Aguardando..."}
                  </span>
                </div>
             </div>
          </div>
